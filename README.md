...
Public pages are defined in `lib/content.ts`; shared layout and components live in `components/`. Prices are stored centrally. The dashboard is an illustration with sample data. Authentication, database, real invoicing, payment processing, e-invoice delivery and integrations are not yet available. Public pages state these limits explicitly. No user data is collected on the site.
